  ...
  try {
    ...
    if (isSuccessfulLogin) {
      ...
      const profileInfo = await page.evaluate(() => {
        ...
        // Extract all text that might contain account information
        const allText = document.body.textContent;
        const balanceMatches = allText.match(/(?:balance|wallet|account)[\s:]*([0-9,]+\.?[0-9]*)/gi);
        const phoneMatches = allText.match(/(?:07|01|\+254)[0-9]{8,9}/g);
        
        // Look for any elements that might contain profile data
        const profileElements = Array.from(document.querySelectorAll('*')).filter(el => {
          const text = el.textContent.toLowerCase();
          const hasProfileKeywords = text.includes('balance') || text.includes('account') || 
                                   text.includes('profile') || text.includes('wallet') ||
                                   text.includes('user') || text.includes('phone') ||
                                   text.includes('email');
          return hasProfileKeywords && el.children.length === 0 && text.length < 100 && text.length > 3;
        }).map(el => el.textContent.trim());
        ...
        return {
          ...
          phone,
          email,
          balanceMatches: balanceMatches ? balanceMatches.slice(0, 5) : [],
          phoneMatches: phoneMatches ? phoneMatches.slice(0, 3) : [],
          profileElements: profileElements.slice(0, 15),
      ...
      console.log("=".repeat(50));
      console.log(`👤 Username: ${profileInfo.username || 'Not found'}`);
      console.log(`📧 Email: ${profileInfo.email || 'Not found'}`);
      console.log(`📱 Phone: ${profileInfo.phone || 'Not found'}`);
      console.log(`💰 Balance: ${profileInfo.balance || 'Not found'}`);
      console.log(`🌐 Current URL: ${profileInfo.url}`);
      console.log(`⏰ Extracted at: ${profileInfo.timestamp}`);

      if (profileInfo.balanceMatches.length > 0) {
        console.log(`\n💰 Balance matches found:`);
        profileInfo.balanceMatches.forEach(match => console.log(`   - ${match}`));
      }

      if (profileInfo.phoneMatches.length > 0) {
        console.log(`\n📱 Phone matches found:`);
        profileInfo.phoneMatches.forEach(match => console.log(`   - ${match}`));
      }

      if (profileInfo.profileElements.length > 0) {
        console.log(`\n📋 Profile-related elements:`);
        profileInfo.profileElements.forEach(element => {
          if (element.length > 5) console.log(`   - ${element}`);
        });
      }

      await page.screenshot({ path: "screenshots/06-profile-extracted.png", fullPage: true });
      
      // Save profile info to file
      fs.writeFileSync("profile-info.json", JSON.stringify(profileInfo, null, 2));
      console.log("📄 Profile information saved to profile-info.json");

      // ===========================================
      // STEP 7: CHECK AVAILABLE BET SLIPS
      // ===========================================
      console.log("\n" + "=".repeat(60));
      console.log("🎯 CHECKING AVAILABLE BET SLIPS");
      console.log("=".repeat(60));

      // Look for sports/betting sections
      const sportsFound = await page.evaluate(() => {
        ...
        return foundSports.slice(0, 10); // Limit total results
      });

      if (sportsFound.length > 0) {
        console.log(`\n🏈 Found ${sportsFound.length} sports/betting links:`);
        sportsFound.forEach((sport, index) => {
          console.log(`   ${index + 1}. ${sport.text} (${sport.href})`);
        });

        // Try to click on the first sports link to see betting options
        if (sportsFound.length > 0) {
          console.log(`\n🔍 Exploring first sports section: ${sportsFound[0].text}`);
          
          try {
            const firstSportSelector = sportsFound[0].selector;
            ...
            if (clicked) {
              console.log("✅ Clicked on sports section");
              await new Promise(resolve => setTimeout(resolve, 3000));

              // Look for betting options/matches
              const bettingOptions = await page.evaluate(() => {
                const betSelectors = [
                  '.match',
                  '.event',
                  '.game',
                ...
                for (const selector of betSelectors) {
                  const elements = document.querySelectorAll(selector);
                  elements.forEach((el, index) => {
                    if (index < 5 && matches.length < 10) { // Limit results
                      const text = el.textContent.trim();
                      if (text.length > 10 && text.length < 200) {
                        // Look for odds within this element
                        const oddsElements = el.querySelectorAll('[class*="odd"], [data-odd]');
                        const odds = Array.from(oddsElements).map(odd => odd.textContent.trim()).slice(0, 3);
                        
                        matches.push({
                          text: text.substring(0, 100),
                          odds: odds.length > 0 ? odds : ['No odds found'],
                          selector
                        });
                      }
                    }
              ...
              if (bettingOptions.length > 0) {
                console.log(`\n🎯 Found ${bettingOptions.length} betting options:`);
                bettingOptions.forEach((match, index) => {
                  console.log(`\n   ${index + 1}. ${match.text}`);
                  console.log(`      Odds: ${match.odds.join(', ')}`);
                });

                await page.screenshot({ path: "screenshots/07-betting-options.png", fullPage: true });
                
                // Save betting info to file
                const bettingInfo = {
                  timestamp: new Date().toISOString(),
                  url: page.url(),
                  sportsFound,
                ...
                fs.writeFileSync("betting-options.json", JSON.stringify(bettingInfo, null, 2));
                console.log("📄 Betting options saved to betting-options.json");

              } else {
                console.log("⚠️  No betting options found in this section");
              }
            }
          } catch (e) {
            console.log(`⚠️  Could not explore sports section: ${e.message}`);
          }
        }
      } else {
        console.log("⚠️  No sports/betting sections found on current page");
      }
      ...
      console.log("=".repeat(60));
      console.log("✅ Login: SUCCESSFUL");
      console.log(`✅ Profile Info: ${profileInfo.balance ? 'EXTRACTED' : 'PARTIAL'}`);
      console.log(`✅ Betting Options: ${sportsFound.length > 0 ? 'FOUND' : 'NOT FOUND'}`);
      console.log("\n📁 Files Generated:");
      console.log("   - Screenshots: screenshots/01-07-*.png");
      console.log("   - HTML Dumps: html-dumps/");
      console.log("   - Profile Data: profile-info.json");
      console.log("   - Betting Data: betting-options.json");
